...
if (isset($_POST['simpan'])) {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array($extFoto, $ext)) {
            ...
            $insert = mysqli_query($koneksi, "INSERT INTO activity (ket_act, foto) VALUES ('$ket_act','$nama_foto')");
        }
    } else {
        $insert = mysqli_query($koneksi, "INSERT INTO activity (ket_act) VALUES ('$ket_act')");
    }


    header("location:activity.php?tambah=berhasil");
}

$id = isset($_GET['edit']) ? $_GET['edit'] : '';
$queryEdit = mysqli_query($koneksi, "SELECT * FROM activity WHERE id='$id'");
   ...
   $ket_act = $_POST['ket_act'];
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array($extFoto, $ext)) {
            ...
            $update = mysqli_query($koneksi, "UPDATE activity SET ket_act='$ket_act', foto='$nama_foto' WHERE id='$id'");
            header("location:activity.php?ubah=berhasil");
        }
    } else {
        //kondisi kalau user tidak ingin memasukkan gambar
        $update = mysqli_query($koneksi, "UPDATE about SET ket_act='$ket_act' WHERE id='$id'");
        header("location:activity.php?ubah=berhasil");
    }
}
?>
                ...
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            <h5><?php echo isset($_GET['edit']) ? 'Edit' : 'Tambah' ?> Activity</h5>
                        </div>
                        <div class="card-body">
                            ...
                            <form action="" method="post" enctype="multipart/form-data">
                                <div class="mb-3 row">
                                    <div class="col-sm-6">
                                        <label for="" class="form-label">Keterangan Activity</label>
                                        <textarea name="ket_act" id="" class="mySummernote form-control"><?php echo isset($_GET['edit']) ? $rowEdit['ket_act'] : '' ?></textarea>
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    <div class="col-sm-12">
